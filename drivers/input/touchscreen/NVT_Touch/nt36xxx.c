...
// Huaqin add for ctp lose efficacy by zhengwu.lu. at 2018/04/18 For Platform
// start
...
// Huaqin add for ctp lose efficacy by zhengwu.lu. at 2018/04/18 For Platform
// end Huaqin add for ZQL1650-1072. by zhengwu.lu. at 2018/04/23  start
...
#if TOUCH_KEY_NUM > 0
const uint16_t touch_key_array[TOUCH_KEY_NUM] = {KEY_BACK, KEY_HOME, KEY_MENU};
#endif
...
int create_gesture_node(void) {
  ...
  if (error) {
    NVT_LOG(
        "[Nvt-ts] : failed to create the gesture_node file in "
...
void destroy_gesture(void) { kobject_put(gesture_kobject); }
...
int32_t CTP_I2C_READ_DUMMY(struct i2c_client *client, uint16_t address) {
  ...
  if (ret < 0) NVT_ERR("CTP_I2C_READ_DUMMY failed.(%d)\n", ret);
...
info_retry:
  ...
  if ((buf[1] + buf[2]) != 0xFF) {
    ...
    if (retry_count < 3) {
      ...
      NVT_ERR(
          "Set default fw_ver=0, x_num=18, y_num=32, abs_x_max=1080, "
          "abs_y_max=1920, max_button_num=0!\n");
      ret = -1;
    }
  } else {
    ret = 0;
...
// Huaqin add for esd check function. by zhengwu.lu. at 2018/2/28  start
  ...
  if (i2c_wr == 0) {  // I2C write
    ...
    if (retries < 20) {
      if (copy_to_user(buff, str, count)) return -EFAULT;
    }
...
static void nvt_parse_dt(struct device *dev) {
  ...
  ts->irq_gpio =
      of_get_named_gpio_flags(np, "novatek,irq-gpio", 0, &ts->irq_flags);
...
static void nvt_esd_check_func(struct work_struct *work) {
  ...
  // Huaqin add for close esd check function log. by zhengwu.lu. at 2017/10/02
  // For Platform start
  // NVT_ERR("esd_check = %d (retry %d/%d)\n", esd_check, esd_retry,
  // esd_retry_max);	//DEBUG

  if (esd_retry >= esd_retry_max) nvt_esd_check_enable(false);
...
static void nvt_ts_work_func(struct work_struct *work) {
  ...
  mutex_lock(&ts->lock);
  ret =
      CTP_I2C_READ(ts->client, I2C_FW_Address, point_data, POINT_DATA_LEN + 1);
  if (ret < 0) {
...
/*
        //--- dump I2C buf ---
        for (i = 0; i < 10; i++) {
                printk("%02X %02X %02X %02X %02X %02X  ", point_data[1+i*6],
   point_data[2+i*6], point_data[3+i*6], point_data[4+i*6], point_data[5+i*6],
   point_data[6+i*6]);
        }
        printk("\n");
*/
// Huaqin add for esd check function. by zhengwu.lu. at 2018/2/28  start
#if NVT_TOUCH_ESD_PROTECT
  // NVT_ERR("%02X %02X %02X\n", point_data[1], point_data[2], point_data[3]);
  // //DEBUG
...
// Huaqin add for ZQL1650-1072. by zhengwu.lu. at 2018/04/23  end
  ...
  if (bTouchIsAwake == 0) {
    ...
    // Huaqin add for ctp lose efficacy by zhengwu.lu. at 2018/04/18 For
    // Platform start
    // enable_irq(ts->client->irq);
    nvt_irq_enable();
    // Huaqin add for ctp lose efficacy by zhengwu.lu. at 2018/04/18 For
  ...
  for (i = 0; i < ts->max_touch_num; i++) {
    ...
    if ((input_id == 0) || (input_id > ts->max_touch_num)) continue;
...
// Huaqin add for esd check function. by zhengwu.lu. at 2018/2/28  start
      ...
      if ((input_x < 0) || (input_y < 0)) continue;
      ...
      if (i < 2) {
        ...
        if (input_p > TOUCH_FORCE_NUM) input_p = TOUCH_FORCE_NUM;
      } else {
        input_p = (uint32_t)(point_data[position + 5]);
      }
...
#if MT_PROTOCOL_B
  for (i = 0; i < ts->max_touch_num; i++) {
    // Huaqin add for  ACTION_HOVER_ENTER 1176453. by zhengwu.lu. at 2018/04/10
    // For Platform start
    if ((press_id[i] != 1) || (finger_cnt <= 0)) {
      // Huaqin add for  ACTION_HOVER_ENTER 1176453. by zhengwu.lu. at
      // 2018/04/10 For Platform end
...
static int8_t nvt_ts_check_chip_ver_trim(void) {
  ...
  for (retry = 5; retry > 0; retry--) {
    ...
    NVT_LOG(
        "buf[1]=0x%02X, buf[2]=0x%02X, buf[3]=0x%02X, buf[4]=0x%02X, "
        "buf[5]=0x%02X, buf[6]=0x%02X\n",
    ...
    for (list = 0;
      ...
      for (i = 0; i < NVT_ID_BYTE_MAX; i++) {
        if (trim_id_table[list].mask[i]) {
          if (buf[i + 1] != trim_id_table[list].id[i]) break;
        }
      }
...
#else
  input_set_abs_params(ts->input_dev, ABS_MT_TRACKING_ID, 0, ts->max_touch_num,
                       0, 0);
#endif  // MT_PROTOCOL_B
...
#else
    ret = request_irq(client->irq, nvt_ts_irq_handler, ts->int_trigger_type,
                      client->name, ts);
#endif
    if (ret != 0) {
...
/* Huaqin add by yuexinghan for gesture mode 20171030 start */
  ...
  nvt_gesture_mode_proc =
      proc_create(NVT_GESTURE_MODE, 0644, NULL, &gesture_mode_proc_ops);
...
long get_gesture_mode(void) { return gesture_mode; }
...
static int32_t nvt_ts_resume(struct device *dev) {
  ...
  // please make sure display reset(RESX) sequence and mipi dsi cmds sent before
  // this
  nvt_bootloader_reset();
...
static int fb_notifier_callback(struct notifier_block *self,
  ...
  if (evdata && evdata->data && event == FB_EARLY_EVENT_BLANK) {
    ...
    if (*blank == FB_BLANK_UNBLANK) {
      // Huaqin add for Reduce the bright screen time by qimaokang at 2018/4/20
      // start
...
static void __exit nvt_driver_exit(void) {
  ...
  if (nvt_wq) destroy_workqueue(nvt_wq);

#if BOOT_UPDATE_FIRMWARE
  if (nvt_fwu_wq) destroy_workqueue(nvt_fwu_wq);
#endif
// Huaqin add for esd check function. by zhengwu.lu. at 2018/2/28  start
#if NVT_TOUCH_ESD_PROTECT
  if (nvt_esd_check_wq) destroy_workqueue(nvt_esd_check_wq);
#endif
  // Huaqin add for esd check function. by zhengwu.lu. at 2018/2/28  end
}
